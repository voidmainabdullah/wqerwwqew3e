...
export const SharedLinks: React.FC = () => {
  ...
  {sharedLinks.map((link, index) => {
    ...
    return (
      <Card
        key={link.id}
        className={`transition-all duration-300 hover:shadow-lg hover:shadow-cyan-500/10 animate-fade-in ${
          inactive ? 'opacity-60' : ''
        } overflow-hidden bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-800 relative`}
        style={{ animationDelay: `${index * 0.1}s` }}
      >
        <CardContent className="p-4 space-y-3">
          {/* Header Section - moved components to top */}
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 border-b border-zinc-800/60 pb-2">
            <div className="flex-1 min-w-0">
              <h3 className="text-base md:text-lg flex items-center truncate font-medium text-white">
          ...
          {/* Short Link and Buttons */}
          <div className="flex flex-col sm:flex-row gap-2">
            <Input
              value={`${window.location.origin}/share/${link.share_token}`}
              readOnly
              className="font-mono text-xs md:text-sm flex-1 min-w-0 bg-zinc-900 text-zinc-100"
            />
            <div className="flex gap-1 sm:gap-2">
              ...
              <Button
                ...
                className="hover:bg-primary/10 hover:text-primary transition-all duration-300 flex-shrink-0"
              >
          ...
          {/* Optional Message */}
          {link.message && (
            <div className="p-2 rounded-xl border border-zinc-800 bg-zinc-900/40">
              <p className="text-xs md:text-sm text-zinc-400 break-words">
                <span className="font-medium text-zinc-200">Message:</span> {link.message}
              </p>
            </div>
          )}

          {/* Info Row */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs md:text-sm text-zinc-400">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                <Switch
                  ...
                  onCheckedChange={(checked) =>
                    toggleSharedLinkStatus(link.id, !checked)
                  }
              ...
              <Button
                ...
                className="text-red-500 hover:text-red-400 hover:bg-red-500/10 transition-all duration-300 hover:scale-105"
              >

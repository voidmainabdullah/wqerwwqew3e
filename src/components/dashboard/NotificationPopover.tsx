...
export const NotificationPopover: React.FC = () => {
  ...
  return <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="icon" className="relative hover :bg-stone-600 bg-transparent ">
          <DotsThreeOutlineVertical className="h-5 w-5 hiiden:md " weight="fill" />
          {unreadCount > 0 && <Badge className="absolute -top-1 -right-1 h-5 w-5 flex items-center justify-center p-0 text-xs" variant="destructive">
      ...
      <PopoverContent className="w-80 p-0" align="end">
        ...
        <ScrollArea className="h-[400px]">
          {notifications.length === 0 ? <div className="p-8 text-center text-muted-foreground">
            ...
            </div> : <div className="space-y-2 p-2">
              {notifications.map(notification => <Card key={notification.id} className={`p-3 ${!notification.read ? 'bg-primary/5' : ''}`}>
                  <div className="space-y-2">
                    ...
                    {notification.type === 'team_invite' && !notification.read && <div className="flex gap-2 mt-2">
                        <Button size="sm" className="flex-1" onClick={() => handleAcceptInvite(notification)}>
                          Accept
                        </Button>
                        <Button size="sm" variant="outline" className="flex-1" onClick={() => handleDeclineInvite(notification)}>
                          Decline
                        </Button>
                      </div>}
                  </div>

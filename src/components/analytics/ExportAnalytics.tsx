  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const { data: userFiles } = await supabase
        .from('files')
        .select('id, original_name, file_size, file_type, created_at, download_count')
        .eq('user_id', user.id);
      ...
      let query = supabase
        .from('download_logs')
        .select('*, files!inner(original_name, file_size, file_type)')
        .in('file_id', fileIds)

...
export const DownloadAnalyticsGraph: React.FC = () => {
  ...
  const groupDownloadsByTime = (downloads: any[], timePeriod: TimePeriod): Map<string, number> => {
  ...
  const formatChartData = (grouped: Map<string, number>, timePeriod: TimePeriod): ChartDataPoint[] => {
    const points: ChartDataPoint[] = [];
    const now = new Date();
    let intervals: number;
    let intervalUnit: 'hours' | 'days';
    ...
    for (let i = intervals; i >= 0; i--) {
      const date = intervalUnit === 'hours' ? subHours(now, i) : subDays(now, i);
      const key = intervalUnit === 'hours' ? format(date, 'yyyy-MM-dd HH:00') : format(date, 'yyyy-MM-dd');
      points.push({
        time: formatTimeLabel(date, timePeriod),
  ...
  const CustomTooltip = ({
    ...
    if (active && payload && payload.length) {
      return <div className="bg-zinc-900 border border-zinc-700 rounded-lg p-3 shadow-xl">
  ...
  if (loading && chartData.length === 0) {
    return <Card className="rounded-xl border border-zinc-700 bg-gradient-to-br from-zinc-900/90 to-zinc-800/50 shadow-lg">
        <CardHeader>
  ...
  return <Card className="rounded-xl border border-zinc-700 bg-gradient-to-br from-zinc-900/90 to-zinc-800/50 shadow-lg hover:shadow-2xl transition-shadow duration-300 py-0">
      <CardHeader>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 bg-zinc-800/60 rounded-lg p-1">
              <Button size="sm" variant={period === '12h' ? 'default' : 'ghost'} onClick={() => setPeriod('12h')} className={period === '12h' ? 'bg-emerald-600 hover:bg-emerald-700 text-white' : 'text-zinc-400 hover:text-white hover:bg-zinc-700'}>
                12H
              </Button>
              <Button size="sm" variant={period === '1d' ? 'default' : 'ghost'} onClick={() => setPeriod('1d')} className={period === '1d' ? 'bg-emerald-600 hover:bg-emerald-700 text-white' : 'text-zinc-400 hover:text-white hover:bg-zinc-700'}>
                1D
              </Button>
              <Button size="sm" variant={period === '1m' ? 'default' : 'ghost'} onClick={() => setPeriod('1m')} className={period === '1m' ? 'bg-emerald-600 hover:bg-emerald-700 text-white' : 'text-zinc-400 hover:text-white hover:bg-zinc-700'}>
                1M
              </Button>
            </div>
            <Button size="sm" variant="ghost" onClick={fetchDownloadData} disabled={loading} className="text-zinc-400 hover:text-white hover:bg-zinc-700">
        ...
        <div className="flex flex-wrap items-center gap-4 mt-4">
          <div className="flex items-center gap-2">
            ...
            {percentChange !== 0 && <Badge className={percentChange > 0 ? 'bg-green-500/10 text-green-400 border border-green-700/30' : 'bg-red-500/10 text-red-400 border border-red-700/30'}>
                {percentChange > 0 ? '+' : ''}
        ...
        <div className="w-full h-80 mt-4">
          <ResponsiveContainer width="100%" height="100%">
            ...
            bottom: 0
              ...
              <XAxis dataKey="time" stroke="#71717a" tick={{
              fill: '#a1a1aa',
              fontSize: 12
            }} tickLine={{
              stroke: '#52525b'
              ...
              <Area type="monotone" dataKey="downloads" stroke="#10b981" strokeWidth={3} fill="url(#downloadGradient)" animationDuration={1000} animationEasing="ease-in-out" />
            </AreaChart>
          </ResponsiveContainer>
        </div>

        {chartData.length > 0 && totalDownloads === 0 && <div className="text-center py-8">

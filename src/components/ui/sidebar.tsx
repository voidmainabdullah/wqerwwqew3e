...
const SidebarProvider = React.forwardRef<HTMLDivElement, SidebarProviderProps>(
  (
    ...
    // setOpen writes cookie and updates either controlled prop or internal state
    const setOpen = React.useCallback(
      (value: boolean | ((v: boolean) => boolean)) => {
        ...
        try {
          ...
          // ignore cookie errors (SSR or strict browsers)
        }
      },
      [open, onOpenChange]
    );
...
/* -------------------------
   Sidebar component
   (Desktop: slide/fade via AnimatePresence & framer motion
    Mobile: Sheet controlled by openMobile)
   ------------------------- */
...
const motionVariants = {
  enter: { x: 0, opacity: 1 },
  exitLeft: { x: -16, opacity: 0 },
  exitRight: { x: 16, opacity: 0 },
  initialLeft: { x: -16, opacity: 0 },
  initialRight: { x: 16, opacity: 0 },
};
...
const Sidebar = React.forwardRef<HTMLDivElement, SidebarProps>(
  ({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, ...props }, ref) => {
    ...
    // if collapsible == "none", simply render a static sidebar container
    if (collapsible === "none") {
      return (
        <div
          ref={ref}
    ...
    // Mobile: render the Sheet (drawer)
    if (isMobile) {
      return (
    ...
    // Desktop: use lightweight motion fade + slight slide
    return (
      <div
        ref={ref}
        ...
        >
          <AnimatePresence initial={false} mode="wait">
            {state === "expanded" && (
              <motion.div
                data-sidebar="sidebar"
                className="flex h-full w-auto flex-col group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow bg-zinc-900"
                initial={side === "left" ? "initialLeft" : "initialRight"}
                animate="enter"
                exit={side === "left" ? "exitLeft" : "exitRight"}
...
/* -------------------------
   Trigger (hamburger) - visible on mobile
   ------------------------- */
...
const SidebarRail = React.forwardRef<HTMLButtonElement, React.ComponentProps<"button">>(({ className, ...props }, ref) => {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear after:absolute after:inset-y-0 after:left-1 after:w-[2px] hover:after:bg-sidebar-border group-data-[side=left]:-right-4 group-data-[side=right]:left-0 sm:flex",
...
/* -------------------------
   Inset main (used with variant=inset)
   ------------------------- */
...
const SidebarContent = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(({ className, ...props }, ref) => {
  return (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-1 flex-col gap-2 overflow-auto group-data-[collapsible=icon]:overflow-hidden scrollbar-thin scrollbar-track-background scrollbar-thumb-muted-foreground/30 hover:scrollbar-thumb-muted-foreground/50",
        className
      )}
      {...props}
    />
...
const SidebarGroup = React.forwardRef<HTMLDivElement, React.ComponentProps<"div">>(({ className, ...props }, ref) => {
  return <div ref={ref} data-sidebar="group" className={cn("relative flex w-full min-w-0 flex-col p-0", className)} {...props} />;
});
...
/* -------------------------
   Menu, Menu Item, Buttons, Actions, Badges, Skeletons
   ------------------------- */
...
/* cva variants preserved for your button styling */
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:!size-9 group-data-[collapsible=icon]:!p-2  [&>svg]:opacity-100 [&>svg]:block [&>svg]:size-4 [&>svg]:shrink-0  [&>span:last-child]:opacity-0 [&>span:last-child]:w-0 [&>span:last-child]:overflow-hidden [&>span:last-child]:transition-all",
  {
    variants: {
      variant: {
...
const SidebarMenuButton = React.forwardRef<HTMLButtonElement, SidebarMenuButtonProps>(
  ({ asChild = false, isActive = false, variant = "default", size = "default", tooltip, className, ...props }, ref) => {
    ...
    const tooltipProps =
      typeof tooltip === "string" ? ({ children: tooltip } as React.ComponentProps<typeof TooltipContent>) : (tooltip as any);
    ...
    return (
      <Comp
        ...
        className={cn(
          ...
          "group-data-[collapsible=icon]:hidopacity-0 group-data-[collapsible=icon]:pointer-events-non",

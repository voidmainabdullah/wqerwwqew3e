...
const DataFlowLine: React.FC<{ className?: string; delay?: number }> = ({ 
  ...
  return (
    <div 
      className={`absolute h-px animate-data-flow bg-gradient-to-r from-transparent via-blue-400/30 to-transparent ${className}`}
...
const PulsingOrb: React.FC<{ className?: string; delay?: number; size?: string }> = ({ 
  ...
  return (
    <div 
      className={`absolute ${size} rounded-full animate-pulse-glow bg-blue-500/20 ${className}`}
      style={{ animationDelay: `${delay}s` }}
    />
  );
};
...
export const AnimatedBackground: React.FC = () => {
  return (
    <div className="absolute inset-0 overflow-hidden bg-gradient-to-r from-black to-black">
      ...
      {/* Pulsing Orbs */}
      <PulsingOrb className="top-12 left-1/2 bg-neutral-800/20" delay={0} size="w-6 h-6" />
      <PulsingOrb className="bottom-12 right-1/2 bg-neutral-800/20" delay={2} size="w-4 h-4" />
      <PulsingOrb className="top-1/3 left-12 bg-neutral-800/20" delay={1} size="w-5 h-5" />
      <PulsingOrb className="bottom-1/4 right-12 bg-neutral-800/20" delay={3} size="w-3 h-3" />
      <PulsingOrb className="top-2/3 right-1/3 bg-neutral-800/20" delay={1.5} size="w-4 h-4" />

      {/* Large Background Orbs for Depth */}
      <div className="absolute top-10 -left-20 w-40 h-40 rounded-full blur-3xl animate-pulse-slow bg-neutral-800/10" />
      <div className="absolute bottom-10 -right-20 w-60 h-60 rounded-full blur-3xl animate-pulse-slow bg-neutral-700/10" style={{ animationDelay: '3s' }} />
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-80 h-80 rounded-full blur-3xl animate-pulse-slow bg-neutral-800/5" style={{ animationDelay: '1.5s' }} />

      {/* Scanning Lines Effect */}
      <div className="absolute inset-0">
        <div className="absolute top-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-neutral-800/50 to-transparent animate-pulse" />
        <div className="absolute top-0 left-0 w-px h-full bg-gradient-to-b from-transparent via-neutral-800/30 to-transparent animate-pulse" />
      </div>
      ...
      <div className="absolute inset-0">
        {Array.from({ length: 20 }).map((_, i) => (
          <div
            key={i}
            className="absolute w-1 h-1 rounded-full animate-pulse bg-neutral-800/30"
            style={{

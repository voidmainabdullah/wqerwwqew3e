...
const Testimonials = () => {
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto space-y-16 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {testimonials.map((testimonial, index) => (
              <motion.div
                key={index}
                className="p-6 rounded-xl border border-border bg-background/80 backdrop-blur-sm hover:border-primary/30 hover:shadow-lg transition-all duration-300"
                initial={{ opacity: 0, y: 30 }}
                animate={
                  inView
                    ? { opacity: 1, y: 0 }
              ...
              >
                <div className="mb-6">
                  {[...Array(5)].map((_, i) => (
                    <span
                      key={i}
                      className="material-icons md-18 mr-1 text-amber-400"
                    >
                      star
                    </span>
                  ))}
                </div>
                <p className="text-lg font-body mb-8 text-foreground/90 italic">
                  "{testimonial.quote}"
                </p>
      ...
      {/* Image Section (separate from testimonials) */}

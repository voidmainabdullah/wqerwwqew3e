...
// Ease-out cubic for ticker animations
...
// Transfer wave visualization
const TransferWave = ({ speed = 0.8, width = 900, height = 120, id = "wave" }) => {
  const waves = useMemo(() => {
    const arr = [];
    for (let i = 0; i < 3; i++) {
      arr.push({ amp: 5 + i * 4, freq: 0.007 + i * 0.0025, phase: i * 0.6 });
    }
    return arr;
  }, []);
...
// AI Organize Management
const AIOrganize = () => {
  const nodes = useMemo(() => Array.from({ length: 6 }).map((_, i) => ({ id: i })), []);
  return (
    <div className="rounded-2xl p-4 bg-white/5 border border-white/6 mb-6">
      <div className="text-sm text-white/70 mb-3">AI Organize Management</div>
    ...
    <section className="relative w-full bg-black text-white overflow-hidden py-16 px-4 sm:px-6 lg:px-12">
      ...
      >
        <motion.div
          ...
          style={{ background: "radial-gradient(circle at 30% 30%, rgba(59,130,246,0.3), rgba(0,0,0,0))", filter: "blur(140px)" }}
          animate={{ x: [0, 18, 0] }}
          transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute right-[-100px] bottom-[-60px] w-[560px] h-[560px] rounded-full"
          style={{ background: "radial-gradient(circle at 70% 70%, rgba(59,130,246,0.2), rgba(0,0,0,0))", filter: "blur(120px)" }}
          animate={{ x: [0, -18, 0] }}
      ...
      <div className="max-w-[1100px] mx-auto relative z-10 space-y-8">
        ...
        <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-white/60 mt-1 max-w-[640px]">Visual file management, integrated metrics, and cinematic system signals for modern distributed teams.</p>
            </div>
          </div>
            ...
            <div className="relative rounded-2xl overflow-hidden">
              <div className="py-4 px-4 sm:px-6">
                <div className="flex flex-col sm:flex-row sm:items-end sm:gap-6">
                  ...
                  <div className="flex-1 flex gap-4 mt-4 sm:mt-0">
                    <div>
                      <div className="text-white text-2xl font-medium">{activeTeams}</div>
                      <div className="text-xs text-white/50 mt-1">Active Teams</div>
                    </div>
                    <div>
                      <div className="text-white text-2xl font-medium">{uptime}%</div>
                      <div className="text-xs text-white/50 mt-1">Uptime</div>
                    </div>
                    <div>
                      <div className="text-white text-2xl font-medium">{processingSpeed}GB/s</div>
        ...
        {/* About / SEO text */}
        <div className="text-xs text-white/40 leading-relaxed mt-8">{seoText}</div>
      </div>
    </section>
  );
}
